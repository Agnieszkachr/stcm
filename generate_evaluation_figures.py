...
def main():
    ...
    ax.hist(cen, bins=14, density=True, color="#4A90D9", edgecolor="white",
            alpha=0.85, label="Observed centred cosine (matched pairs)")
    ...
    ax.axvline(cen.mean(), color="#2ECC71", lw=2, linestyle="--",
    ...
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.barh(range(5), resid[:5], color="#4A90D9", edgecolor="white", alpha=0.85)
    ax.axvline(SIG_B_MEAN, color="#E74C3C", lw=2, linestyle="--",
    ...
    # Figure 3: full ranking
    fig, ax = plt.subplots(figsize=(8, 9))
    colors = ["#E74C3C" if v >= cen.mean() else "#95A5A6" for v in cen]
    ax.barh(range(len(cen)), cen, color=colors, edgecolor="white", alpha=0.85)
    ax.set_yticks(range(len(cen)))
    ax.set_yticklabels(labels, fontsize=7)
    ax.invert_yaxis()
    ax.axvline(cen.mean(), color="#2ECC71", lw=1.5, linestyle="--",
    ...
    ax.scatter(lv, cen, color="#4A90D9", edgecolor="white", s=60, zorder=3)
    ...
    for i, vals in enumerate(data, 1):
        ...
        ax.scatter(xs, vals, alpha=0.7, color="#4A90D9", zorder=3)
    ax.set_ylabel("Mean-centred cosine")
